...
async function Login() {
	try {
		...
		let dataTowns=response.match(/"id":"[^"]*","name":"[^"]*","playing_on":true/igm)
		let myTowns=[];
		for(let town of dataTowns){
			myTowns.push(JSON.parse("{" + town + "}"));
		}

		let h_token =response.match(/CSRF.token = '[^;]*';/igm)[0].split("'")[1];
		let playerName = response.match(/Login.player_name = '[^;]*';/igm)[0].split("'")[1];
		...
		//   console.log(world,world.headers);
		  ...
		  const login_startup_time = world.headers.get("set-cookie").match(/login_startup_time=[^%]*/ig)[0].split("=")[1]
		  ...
		  let worldIndex = await fetch(session.headers.get("location"), {
			...
			h_token =worldIndexData.match(/csrfToken":"[^"]*/igm)[0].split('"')[2];

		let ts=session.headers.get("location").match(/ts=.*/ig)[0].split("=")[1]
		  console.log(ts,login_startup_time);
			let town_data= await fetch("https://gr81.grepolis.com/game/data?town_id=7718&action=get&h="+h_token, {
				"headers": {
			...
			let farm_towns = [];
			  ...
			  town_data=town_data['json']['map']['data']['data']['data']['0']['towns'];
			  for(let farm in town_data){
				  if(town_data[farm]['relation_status']){
					farm_towns.push(town_data[farm]);
				  }
				}
